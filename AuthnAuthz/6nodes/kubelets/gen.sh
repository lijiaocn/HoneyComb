...
choose(){
	func_yellow_str "`ls ./kubelets/`"
	echo -n "Choose the kubelet:  "
	read KUBELETE

	KUBELETEDIR=./kubelets/${KUBELETE}

	if [ ! -d  ${KUBELETEDIR} ];then
		func_red_str "Not found directory ${KUBELETE} in ./kubelets"
		exit
	fi

	gen_kubelet_cert ${KUBELETEDIR} "`pwd`/${CA}" "`pwd`/${CAKEY}"  `pwd`/${OUTPUT}
}

gen_all(){
	for kubelet in `ls ./kubelets/`
	do
		func_yellow_str "Generate cert for kubelets/${kubelet}"
		gen_kubelet_cert ./kubelets/${kubelet} `pwd`/${CA} `pwd`/${CAKEY} `pwd`/${OUTPUT}
	done
}
