...
choose(){
	func_yellow_str "`ls ./registry/`"
	echo -n "Choose the registry:  "
	read KUBELETE

	KUBELETEDIR=./registry/${KUBELETE}

	if [ ! -d  ${KUBELETEDIR} ];then
		func_red_str "Not found directory ${KUBELETE} in ./registry"
		exit
	fi

	gen_registry_cert ${KUBELETEDIR} "`pwd`/${CA}" "`pwd`/${CAKEY}"  `pwd`/${OUTPUT}
}

gen_all(){
	for registry in `ls ./registry/`
	do
		func_yellow_str "Generate cert for registry/${registry}"
		gen_registry_cert ./registry/${registry} `pwd`/${CA} `pwd`/${CAKEY} `pwd`/${OUTPUT}
	done
}
