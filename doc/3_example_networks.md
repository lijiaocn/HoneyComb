...
# 3_example_networks
创建时间: 2015/07/12 09:52:53  修改时间: 2015/07/14 17:03:38 作者:lijiao
...
Ingress IP从外部的LB获得，具体的获取方式取决于外部LB的实现。我们可以构想出下面的拓扑: 
	  ...
	  ----------------------隔----------离-+-----------带--------      |      -------------
	              ...
	              最高安全级别区域        隔     +---------------------'------------------+
	                                       |     |                 Load Balancer          |
	  3. 控制器与Node之间有严格的网络管制  |     +----------------------------------------+
	                                       |
	                  +---------+          |         +---------+  +---------+  +---------+
	                  |ApiServer|          离---+    |   Node  |  |   Node  |  |   Node  |
	                  |         |               |    |         |  |         |  |         |
	                  +---------+               |    +---------+  +---------+  +---------+
	           +---------+     +---------+           +---------+  +---------+  +---------+
...
现在Kubernetes是利用iptables和用户态的转发程序(后续将只使用iptables)实现报文的转发，这样必然会拉低网络性能。
