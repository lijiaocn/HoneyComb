...
# 3_example_networks
创建时间: 2015/07/12 09:52:53  修改时间: 2015/07/13 22:06:24 作者:lijiao
...
2015-07-14 17:06:06   增加了“场景1->LoadBalancer方式”、“场景1->问题与改进”、“场景2”、“场景3”
...
有了EndPoints之后，我们就可以向下面这样充实我们在上一节中假想的拓扑(增加了4和5):
	                               ...
	                               1. 只有LB是暴露在公网的                        5. 对第三方服务的请求经过NAT后到达目的地址
	                                                                                                  +-------------------+
	                                                                                                  |                   |
	                                                                                          +---|>  | Other Companies's |
	                                  Internet                        |                       |       |      services     |
...
Kubernetes只提出这种要求，完全不关心要怎样实现，这个问题其实是部署Kubernetes的用户自己需要考虑的问题。

这就是一个开放性问题了，可以有各种各样的实现。找任何一家网络厂商，他们都会告诉你他们的独特之处。即使不想依赖于硬件，各种开源的sdn方案也为数不少。
